...
dis_folder <- "../17.figure_7.19/Family/spectra_dis/"
...
write.table(DIS, "Figure_7_20a.txt", quote = F, sep = "\t", row.names = F)
...
dis_folder <- "../17.figure_7.19/Network_cluster/spectra_dis/"
dis_files <- list.files(path = dis_folder, pattern = "_mean.txt",
                        full.names = TRUE, recursive = FALSE)
...
write.table(DIS, "Figure_7_20b.txt", quote = F, sep = "\t", row.names = F)

p <- plot_grid(p_a, p_b, ncol = 1, labels = "auto", rel_heights = c(1.3, 1))
ggsave("Figure_7_20.pdf", p, width = 11, height = 7)
