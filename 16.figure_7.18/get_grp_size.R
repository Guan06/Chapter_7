...
### for family features
grp_file <- "../14.figure_7.16/CAS_ASV_1774_tax.txt"
...
write.table(df_size, "size_families.txt", quote = F, sep = "\t", row.names = F)
...
colnames(df_size) <- c("Group", "Number_of_ASVs", "Compartment")
write.table(df_size, "size_network_clusters.txt", quote = F, sep = "\t", row.names = F)
