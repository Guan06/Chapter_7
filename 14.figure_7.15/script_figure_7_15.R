...
#### for family ###############################################################
dis_mean_file <- "../12.figure_7.13//Family_single_pm_dis_mean.txt"
...
p_a <- ggdraw(aligned_plots[[1]]) + draw_plot(aligned_plots[[2]])

write.table(DIS, "Figure_7_15a.txt", quote = F, sep = "\t", row.names = F)

######## for network cluster ##################################################
dis_mean_file <- "../12.figure_7.13//Cluster_single_pm_dis_mean.txt"
...
write.table(DIS, "Figure_7_15b.txt", quote = F, sep = "\t", row.names = F)

p <- plot_grid(p_a, p_b, ncol = 1, labels = "auto", rel_heights = c(1.3, 1))
ggsave("Figure_7_15.pdf", p, width = 11, height = 7)
